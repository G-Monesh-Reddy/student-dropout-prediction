...
def rule_based_counselling(prediction):
    if prediction == "Dropout":
        return (
            "Student is at risk of dropout. Academic performance should be improved, "
            "pending subjects should be cleared, and mentoring support is recommended."
        )
    else:
        return (
            "Student performance is satisfactory. Encourage consistent study habits "
...
if mode == "Faculty Mode (Upload File)":
    ...
    st.caption("ℹ️ Please enter accurate academic details. All inputs are mandatory.")

    col1, col2 = st.columns(2)

    with col1:
        g1 = st.number_input("1st Semester Grade (0–10)", 0.0, 10.0, 7.0, 0.1)
        a1 = st.number_input("1st Semester Approved Subjects", 0, 10, 4)
        g2 = st.number_input("2nd Semester Grade (0–10)", 0.0, 10.0, 7.0, 0.1)
    ...
    with col2:
        ...
        debtor = st.selectbox("Outstanding Dues (Debtor)", [0, 1], format_func=lambda x: "No" if x == 0 else "Yes")
